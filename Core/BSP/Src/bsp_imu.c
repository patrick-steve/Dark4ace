...
uint8_t gyro_init() {
	...
	HAL_Delay(2);
#ifdef G_X_OFFSET
	imu_data.gx_offset = G_X_OFFSET;
#else
	imu_data.gx_offset = 0;
#endif
#ifdef G_Y_OFFSET
	imu_data.gy_offset = G_Y_OFFSET;
#else
	imu_data.gy_offset = 0;
#endif
#ifdef G_Z_OFFSET
	imu_data.gz_offset = G_Z_OFFSET;
#else
	imu_data.gz_offset = 0;
#endif
	return 0;
	//blah
}
...
void gyro_offset_cali() {
	...
	int32_t curr_mag = 100;
	while (curr_mag > 20){

		gyro_read_bytes(BMI088_GYRO_X_L, gyro_buff, 6);
		curr_mag = 0;
		curr_mag += (int16_t)(gyro_buff[1] << 8 | gyro_buff[0]);
		curr_mag += (int16_t)(gyro_buff[3] << 8 | gyro_buff[2]);
		curr_mag += (int16_t)(gyro_buff[5] << 8 | gyro_buff[4]);
		HAL_Delay(1);
	}
	...
	imu_data.gx_offset = gyro_x_total / 50;
