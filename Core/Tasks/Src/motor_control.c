...
void angle_pid(double setpoint, double curr_pt, motor_data_t *motor) {
	...
	float p_out = motor->angle_pid.kp * ang_diff;
	float d_out = motor->angle_pid.kd * (motor->angle_pid.error[0] - motor->angle_pid.error[1]);

	motor->angle_pid.integral += motor->angle_pid.error[0] * motor->angle_pid.ki;
	float_minmax(&motor->angle_pid.integral, motor->angle_pid.int_max, -motor->angle_pid.int_max);
	float i_out = motor->angle_pid.integral;

	motor->angle_pid.output = p_out + d_out + i_out;
...
void speed_pid(double setpoint, double curr_pt, pid_data_t *pid) {
	...
	uint32_t time_diff = (pid->last_time[0] - pid->last_time[1])*1e-6;
	...
	float_minmax(&final, pid->max_out, -pid->max_out);
	pid->output = final;
