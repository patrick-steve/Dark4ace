...
void accel_data_ready(accel_data_t accel_data) {
	...
	accel_data.ay = 0;
		...
		accel_proc_data.ay = accel_data.az;
...
void quat_startup() {
	...
	tempq0 = q0 + (-q1 * gx - q2 * gy - q3 * gz) * 1;
	tempq1 = q1 + (q0 * gx + q2 * gz - q3 * gy) * 1;
	tempq2 = q2 + (q0 * gy - q1 * gz + q3 * gx) * 1;
	tempq3 = q3 + (q0 * gz + q1 * gy - q2 * gx) * 1;
...
//	if (IMU_ORIENTATION == 0 || IMU_ORIENTATION == 2) {
...
#endif
//	} else {
//		//flip pitch and roll when board is rotated 90 degrees
//		/* yaw    -pi----pi */
//		imu_heading.yaw = -atan2(2 * q1 * q2 + 2 * q0 * q3,
//				-2 * q2 * q2 - 2 * q3 * q3 + 1) * IMU_YAW_INVERT;
//		/* roll   -pi----pi  */
